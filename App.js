...
export default function App() {
  ...
  function syncData(syncTime) {
    if(syncTime != undefined)
      Axios.get(`http://localhost:9000/dictionary/sync/${syncTime}-0`)
      .then((res) => {
  ...
  function submitSearch(input) {
    db.transaction(tx => {
      ...
      (txObj, resultSet) => resultSet.rows[0] != null ? console.log(resultSet.rows[0]) : {},
      (txObj, err) => console.log(err))
    })
  }
...
/*
  useEffect(() => {
    ...
    db.transaction(tx => {
      ...
      , null, 
      (txObj, resultSet) => {syncData(resultSet.rows[0].times)},
      (txObj, err) => console.log(err))
    })
    queryData()
  }, [])*/
